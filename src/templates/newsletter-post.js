...
export default function LetterPost({ data }) {
    ...
    <div className={styles.post}>
          ...
          */}
          ...
          <div className="featured" style={{marginBottom: "30px"}}>
            {/* <hr /> */}
            <h2 style={{color: "var(--secondary)", marginBottom: "-5px", fontSize: "1em"}}>{format('{1} {0}, {2}', dd, mm, yy) } <a className="Tag" href="http://magnus.so/newsletter/">{category}</a></h2>
...
query LetterPost($slugNew: String) {
  markdownRemark(frontmatter: {slugNew: {eq: $slugNew}}) {
    html
    frontmatter {
      title
      date
